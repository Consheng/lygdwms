...
class Stock_GroupDialogActivity : BaseDialogActivity() {
    ...
    override fun initData() {
        ...
        if (bundle != null) {
            ...
            if(bundle.containsKey("stockPosition") && bundle.getSerializable("stockPosition") != null) {
                stockPos = bundle.getSerializable("stockPosition") as StockPosition
            ...
            if(!stockEnable && stock != null) { // 仓库不能选择
                ...
                // 是否启用了库区
                if(stock!!.useStockArea == 1) {
                    smqFlag = '2'
                } else if(stock!!.useStorageRack == 1) { // 是否启用了货架
                    smqFlag = '3'
                } else if(stock!!.useStockPosition == 1) { // 是否启用了库位
                    smqFlag = '4'
                }
            }
        }
...
//                showForResult(CaptureActivity::class.java, BaseFragment.CAMERA_SCAN, null)
            ...
            R.id.btn_confirm -> { // 确认
                ...
                if(stock!!.useStockArea == 1 && stockArea == null) { // 是否启用了库区
                    Comm.showWarnDialog(context, "请选择库区！")
                    return
                }
                if(stock!!.useStorageRack == 1 && storageRack == null) { // 是否启用了货架
                    Comm.showWarnDialog(context, "请选择货架！")
                    return
                }
                if(stock!!.useStockPosition == 1 && stockPos == null) { // 是否启用了库位
    ...
    private fun sendObj() {
        ...
        if(stockPos != null) {
            intent.putExtra("stockPosition", stockPos as Serializable)
        }
